...
{
  // add an almighty ceph.client.admin.keyring
  KeyRing keyring;
  ...
  EntityName name;
  name.set(CEPH_ENTITY_TYPE_CLIENT, "admin");
  auth_inc.name = name;

  EntityAuth auth;
  if (!keyring.get_auth(auth_inc.name, auth)) {
    cerr << "key for " << auth_inc.name << " not found in keyring: "
         << keyring_path << std::endl;
    return -EINVAL;
  }
  auth_inc.auth.key = auth.key;
  ::encode(string("allow *"), auth_inc.auth.caps["mon"]);
  ::encode(string("allow *"), auth_inc.auth.caps["mds"]);
  ::encode(string("allow *"), auth_inc.auth.caps["osd"]);
